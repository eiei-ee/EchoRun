...
- 回声契约：AI 将路线偏好、跳滑倾向或固定节奏转化为每代不同的赛道规则与反制目标
...
首局是校准局；只有总样本、有效动作数量和动作类型覆盖同时达到阈值，下一局才会出现半透明的个人 AI 影子，单纯保持直跑不能完成校准。每局开始时同时冻结上一代行为模型与玩家风格快照，并从路线、跳滑倾向或动作节奏中生成一份人类可读的“回声契约”。契约会移动高价值路线、指定反向跳滑组合或交替障碍节拍；采取反制行为获得推进，重复已被识别的旧习惯则会强化回声。影子不使用物理碰撞推挤玩家，但拥有独立的赛道坐标、障碍感知和逻辑碰撞。本局新动作只训练下一代，因此影子不会即时复制当前输入。

核心循环为：`校准自己 → AI 生成个人回声与契约 → 识别它继承的习惯 → 改变行为破解契约并跑赢回声 → 下一代重写规则`。赛前界面说明 AI 学到的特征、本代规则和目标；HUD 显示契约进度与“AI施压/恢复/反制生效”；赛后报告展示本代习惯、完成结果与下一代学习方向。距离领先但未完成契约仍判定失败。去掉运行时 AI 后，个人对手、契约规则、代际成长和正式胜负都会消失。
